...
- [ ] **`story` skill — first DRAFT written** ([`skills/story/SKILL.md`](skills/story/SKILL.md), 2026-09-06, not symlinked/activated). Next: run it on the *next* story to exercise it, then resolve its open questions (viz-plan file split, craft-doc bundling, portability) and consider activating. [#10](https://github.com/datasets/datapressr/issues/10)
- [ ] **`enrich` — reps then draft.** Do 1-2 "descriptive stats + first charts" passes by hand on co2-ppm and oil-prices (Observable Plot per `docs/charting.md`), then draft `skills/enrich/SKILL.md` the way `story` was drafted. Genuinely under-evidenced today — no discrete enrich pass has ever been done.
