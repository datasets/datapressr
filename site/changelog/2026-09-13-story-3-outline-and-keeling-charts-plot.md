...
**Keeling curve charts moved to Observable Plot**, replacing the original hand-rolled SVG generator and bringing story #1 in line with the [charting decision](https://github.com/datasets/datapressr/blob/main/docs/charting.md) story #2 already uses. The 350 ppm and 400 ppm reference lines are now computed from the plotted annual series itself rather than hardcoded, and labelled as annual-mean crossings (1988, 2015) — the well-known 1986 and 2013 dates mark the first single *month* above each threshold, a materially different fact from the annual mean this chart shows. The monthly/seasonal chart now marks the year's actual high and low (May and September) directly on the line.
