...
- **A normalised ranged-bar chart by hand was about the ceiling of what inline SVG is comfortable for** — one clipped bar (genetic diversity at 16.6×) already needed special handling. This was the chart that settled the tooling question: both charts here are now authored with [Observable Plot](https://observablehq.com/plot/) and rendered to static SVG at build time. See [`docs/charting.md`](../../docs/charting.md) and the [side-by-side](../charting-spike.html).
