...
Both rendered by `oil-prices-make-charts.mjs` with Observable Plot per [`docs/charting.md`](https://github.com/datasets/datapressr/blob/main/docs/charting.md) — annotate directly on the charts rather than relying on a legend.

## Voice

Plain and factual, per [`docs/voice-guide.md`](https://github.com/datasets/datapressr/blob/main/docs/voice-guide.md). Let the two numbers on the same day — $17.36 and -$36.98 — carry the contrast, and let Brent's $9.12 the next day keep it honest. No "unprecedented", no "the day the market broke". The futures/storage explanation is attributed outside context, not a lecture and not a claim this data proves. A "sounds like me" pass is a separate step the author runs.
