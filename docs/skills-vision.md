...
**Charting, further direction (2026-08-30), tracked in [#11](https://github.com/datasets/datapressr/issues/11):** two tracks. *Near-term:* just build charts that look really good and are easy to make — hand-rolled inline SVG, standalone HTML files, or JS injected via a page's frontmatter/head (Flowershow serves all three) — mix freely, iterate on look and feel. Do **not** block on a standard. *Deferred (longer investigation):* settle a standard chart mechanism that DataHub and/or Flowershow support out of the box, so stories stop being bespoke; needs real surface-area exploration plus a couple more hand-made stories' friction first. Spike 1 below is that deferred track.
...
1. **Charting standard** ([#11](https://github.com/datasets/datapressr/issues/11), deferred track) — informed by 2–3 hand-written stories, not before. Survey Vega-Lite vs Observable Plot (vs others); Flowershow's native chart support; DataHub's `views` array (where it's enough, where it isn't); decide who renders it and how a chart travels (with the dataset, with the story, or both). Near-term, ignore this and just make good charts pragmatically.
...
4. **Unattended cloud execution** — where could the pipeline run without a human kicking off `node build.ts`? Blocks a real `monitor` skill; also enables CI builds on source change. Survey Cloudflare Workers/Workflows/R2 vs. the low-ceremony GitHub Actions option. Tracked in [#6](https://github.com/datasets/datapressr/issues/6).
...
(`/validate`'s schema/license/source checks are already extended — see the changes to `skills/validate/SKILL.md` above — so that item is done rather than upcoming.)
