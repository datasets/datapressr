...
>
> - Compare the two external Project Drawdown packages and prepare a recommendation — `datapressr-jh6`. Working notes: [`docs/project-drawdown-comparison.md`](https://github.com/datasets/datapressr/blob/main/docs/project-drawdown-comparison.md).
...
**Stale because** the issue presents the near-term policy as open ("hand-rolled inline SVG … raw HTML … pick whatever gets a good-looking chart fastest"). It is decided and written down: [`docs/charting.md`](https://github.com/datasets/datapressr/blob/main/docs/charting.md) (2026-09-06) settles on Observable Plot rendered to static SVG at build time for stories, and declarative `views` for dataset pages. All three stories now follow it; the Keeling charts were ported from hand-rolled SVG (`datapressr-8rk`). The issue's *second* track — a chart mechanism DataHub or Flowershow support natively — is genuinely still open and is `datapressr-7fs` (deferred, post-v1).

**Proposed addition:**

> **Update 2026-09-18.** The near-term track is **decided and no longer open to mix-and-match**: Observable Plot rendered to static SVG at build time for stories, declarative `views` for dataset pages. Written up in [`docs/charting.md`](https://github.com/datasets/datapressr/blob/main/docs/charting.md). All three stories follow it, and the Keeling charts were ported over from hand-rolled SVG (`datapressr-8rk`).
>
...
> 1. ~~Prove `structure` in anger~~ — done (#8, co2-ppm); benchmarked twice ([`docs/structure-benchmark.md`](https://github.com/datasets/datapressr/blob/main/docs/structure-benchmark.md)) — round 1 across three sources with eight fixes applied, round 2 across a JSON/REST API source and a relational multi-file join.
...
> 3. ~~Charting~~ — near-term decided: Observable Plot → static SVG for stories, `views` for dataset pages ([`docs/charting.md`](https://github.com/datasets/datapressr/blob/main/docs/charting.md)). Native support remains deferred (#11).
