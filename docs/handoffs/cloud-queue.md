...
Final verification: `npm test` 107/107; `npm run check:invisible` clean; `validate-datapackage.mjs` 0 errors and 0 warnings on all five datasets; working tree clean, everything pushed to `main`.

### New work found during this run (for the owner to file as Beads)

1. **`scripts/validate-datapackage.mjs:125` has the same silent-exit-0 defect** that finding 4 above fixed in the new script — `import.meta.url === ` + "`" + `file://${process.argv[1]}` + "`" + ` compares a percent-encoded URL against a raw path, so `/validate` run from a directory with a space or a non-ASCII character in its name prints nothing and exits 0, which reads as "passed". The fix is one line (`pathToFileURL(process.argv[1]).href`). Not done here because the file has two per-dataset copies that the repo's convention keeps byte-identical, so it is a three-file change outside this task's scope — but it is a real defect in a user-facing command, and the more dangerous of the two instances.
