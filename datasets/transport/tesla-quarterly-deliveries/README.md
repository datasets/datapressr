...
**Deliveries** are vehicles transferred to a customer with the paperwork complete. **Production** is vehicles built. They are two separate measures, they are never equal, and neither is revenue. Releases through Q4 2021 called the delivery count "slightly conservative" and said final numbers "could vary by up to 0.5% or more"; releases from Q1 2022 onward drop that language entirely.

**Over this range Tesla does not report individual models.** It reports groups, and the grouping has changed twice. (The earlier prose releases *did* give per-model figures — see below — so this is a property of the table era, not of Tesla's reporting for all time.)
...
Coverage starts at **Q2 2019** because that is the first release to publish its figures in a table. 19 earlier releases were found, covering **18 quarters between Q1 2013 and Q1 2019** — the run is not contiguous, and no delivery 8-K was filed at all for Q2 2013 through Q4 2014. They state the numbers in prose — "Q1 deliveries totaled 29,980 vehicles, of which 11,730 were Model S, 10,070 were Model X, and 8,180 were Model 3" — with the wording changing release to release. Those filings are archived and listed in `data/source-filings.csv` with `layout: prose` and `extracted: false`. They are a known gap, deliberately left rather than extracted by a set of one-off regexes whose failures would be silent. Note what that gap costs: the prose releases break out **individual models** (`Model S`, `Model X`, `Model 3` separately), so extracting them would add finer granularity, not just more quarters. `source-filings.csv` accounts for all 50 archived exhibits — 29 table, 19 prose, 2 exhibits that are not delivery releases at all — so what was found and not used is visible rather than absent.
...
- **Three periods are covered by more than one exhibit**: Q2 2017 was filed twice, an unrelated exhibit was filed the day before the Q3 2018 release, and the Q4 2022 filing carried a second exhibit announcing Investor Day. The build applies a preferred-version rule — the later filing wins, both snapshots stay in `archive/` — but no quarter actually triggers it, because in each case only one exhibit carries a table.

## Restatements: the annual recap does not match the four quarters

`data/tesla-annual-deliveries.csv` holds the full-year figures that Tesla recaps in each Q4 release. It covers **2020–2025**: the Q4 2019 release carried no full-year recap, so the annual series starts a year after the quarterly one. They are kept in a separate resource because **they do not reconcile with the four quarters as originally reported**:
