...
**Location in this repo:** the `changelog/` folder above lives at `site/changelog/`, not the repo root, so the entries publish on the site (`site/` git-autosyncs to the Flowershow site on every push to `main`). Read every `changelog/` path above as `site/changelog/`, put images in `site/changelog/images/`.
