...
Capture the license and source URL *now*, while you have the source open — not as a follow-up. This is the single most-forgotten step (see the rigor pass in `docs/plans/skills-vision.md`). If the license genuinely isn't stated anywhere, say so explicitly rather than leaving `licenses` empty and moving on.
