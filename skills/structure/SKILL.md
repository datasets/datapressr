...
- **Trim whitespace on every string you keep.** `"Sub-Saharan Africa "` and `"Sub-Saharan Africa"` are two different keys and one real region; in `population-growth` the trailing spaces made exactly two regions' observation names disagree with their own metadata names. Trim once, centrally, not per column when a mismatch surprises you.
