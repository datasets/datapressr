...
**Update:** Stage 1 worked — 20+ datasets published to DataHub from the backlog. `/init`/`/validate`/`/push` cover the last mile, and `structure` (turning a raw source into a clean, typed dataset) is now a real skill too — see [`docs/plans/skills-vision.md`](docs/plans/skills-vision.md). `/validate` and the `structure` playbook's cleanup idioms are backed by an actual test suite (`npm test`), not just prose.

The backlog/inbox/dashboards markdown files are superseded. Beads (`bd ready`) tracks actionable work; [GitHub issues](https://github.com/datasets/datapressr/issues) retain lightweight captures and historical discussion. Start a working session with [NEXT.md](NEXT.md).

**Next:** `capture`/`archive` skills, then a few data stories written by hand before deciding on a charting approach — see `docs/plans/skills-vision.md`.
