...
**Update:** Stage 1 worked — 20+ datasets published to DataHub from the backlog. `/init`/`/validate`/`/push` cover the last mile, and `structure` (turning a raw source into a clean, typed dataset) is now a real skill too — see [`site/docs/skills-vision.md`](site/docs/skills-vision.md). `/validate` and the `structure` playbook's cleanup idioms are backed by an actual test suite (`npm test`), not just prose.
...
**Next:** `capture`/`archive` skills, then a few data stories written by hand before deciding on a charting approach — see `site/docs/skills-vision.md`.
